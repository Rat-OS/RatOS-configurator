/**
 * From https://github.com/wd-David/esbuild-plugin-pino/blob/main/src/index.ts
 * Modified to avoid bad use of absolute paths.
 */
...
type NewEntrypointsType = Extract<BuildOptions['entryPoints'], { in: string; out: string }[]>;
 ...
 * @returns
 */
function isStringArray(entryPoints: BuildOptions['entryPoints']): entryPoints is string[] {
 ...
 * @returns
 */
function transformToObject(entryPoints: string[], outbase: string | undefined): Record<string, string> {
	...
	if (!outbase) {
		...
		} while (entryPoints.every((entrypoint: string) => entrypoint.startsWith(`${nextOutbase}${separator}`)));
	}
 ...
 * @returns
 */
function transformToNewEntryPointsType(entryPoints: Record<string, string>): NewEntrypointsType {
...
export default function esbuildPluginPino({ transports = [] }: { transports: string[] }): Plugin {
	return {
		name: 'pino',
        ...
        `;
				...
				const code = globalThisDeclaration;
