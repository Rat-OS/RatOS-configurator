...
export const initBeltTensionChartAnnotations = (
	...
	const annotations = pairs.map(([p1, p2], i) => {
		if (
			...
			surface.addAnimation(annotationAnimation1, annotationAnimation2);
...
export const removeBeltTensionChartAnnotations = (surface: SciChartSurface | null, fromIndex: number = 0) => {
	if (surface == null) {
		return;
	}
	let i = fromIndex;
...
export const updateBeltTensionChartAnnotations = (
	surface: SciChartSurface | null,
	sequencePair: [SequenceData, SequenceData] | null,
	pairedPeaks: PeakPairingResult | null,
) => {
	if (sequencePair == null || pairedPeaks == null || surface == null) {
		removeBeltTensionChartAnnotations(surface);
		return;
	}
	const [seq1, seq2] = sequencePair;
	...
	// Remove old pairs
	removeBeltTensionChartAnnotations(surface, foundPairs);
};
